...
from modules import Encoder, MoCo
...
def pretrain_epoch(model, 
    ...
    for step, pre_index in enumerate(range(data.shape[0] // batch_size + 1)):
        ...
        if flag == 'aug_nn' or flag == 'aug_nn_with_att':   # Using its augmentation counterpart and neighbor to form positive pairs
...
def pretrain(args, device):
    data, label, cell_type, neighbors, adata_hvg, _ = data_process(args.root_dir, 
                                                                    args.data_type,
                                                                    args.dataset_name,
                                                                    args.num_genes,
                                                                    k=args.n,
    ...
    return data, label, cell_type, model, t0
