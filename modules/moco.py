...
class MoCo(nn.Module):
    ...
    def _dequeue_and_enqueue_2(self, keys):
        ...
        if self.ptr + batch_size >= self.K:
            # self.queue[self.ptr:, :] = keys[:self.K - self.ptr, :].detach()
            self.queue[self.ptr:, :] = keys[batch_size + self.ptr - self.K:, :].detach()
            self.ptr = 0
        else:
            self.queue[self.ptr: self.ptr + batch_size, :] = keys.detach()
            self.ptr = (self.ptr + batch_size) % self.K
    ...
    def forward_aug_nn_with_att(self, x1, x2):
        q = self.encoder_q(x1)
        latent = self.encoder_q.get_embedding(x1)
        q = F.normalize(q, dim=1)
        ...
        pos_sim1 = torch.einsum("ic, ic -> i", [q, k1]).unsqueeze(-1)
        pos_sim2 = torch.einsum("ic, ic -> i", [qc, k2]).unsqueeze(-1)
        pos_sim2 = pos_sim2.reshape(-1, c)

        assert pos_sim2.size(0) == pos_sim1.size(0)

        pos_sim = torch.cat([pos_sim1, pos_sim2], dim=1)
        alphas = self._softmax(pos_sim)
        # pos_sim = self._scores(alphas=alphas, pos_sim=pos_sim)
        pos_sim =  1 / self.lam * self._scores(alphas=alphas, pos_sim=pos_sim)
        neg_sim = torch.einsum("ic, jc -> ij", [q, self.queue.clone().detach()])
        ...
        # self._dequeue_and_enqueue(k2)
        self._dequeue_and_enqueue_2(k2)
    ...
    def _scores(self, alphas, pos_sim):
        return torch.mul(alphas, pos_sim)
    
    def forward(self, x1, x2, flag="aug_nn"):
        if flag == 'aug_nn':
            return self.forward_aug_nn(x1, x2)
        elif flag == 'aug_nn_with_att':
            return self.forward_aug_nn_with_att(x1, x2)
