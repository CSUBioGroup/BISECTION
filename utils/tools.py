...
def getGeneSetMatrix_2(_name, genes_upper, gene_sets_path):
    if _name[-3:] == 'gmt' or _name[-3:] == 'txt':
        print(f"GMT or TXT file {_name} loading ... ")
        filename = _name
        filepath = os.path.join(gene_sets_path, f"{filename}")
        ...
        new_pathway2gene = {k: v for k, v in pathway2gene.items() if len(v) < 1000}
        pathway2gene = new_pathway2gene
        print(f"length of pathways after delete the gene set containing more than 1000 genes: {len(pathway2gene)}")
